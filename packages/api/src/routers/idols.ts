...
export const idolsRouter = router({
  ...
  battlePair: publicProcedure.input(z.object({ sessionId: z.string() })).query(async () => {
    const db = createDb();
    ...
    return {
      idolA: {
        ...
        name: idolA.name,
        group: idolA.group,
        photo: photoA ? { id: photoA.id, imageUrl: photoA.imageUrl } : { id: null, imageUrl: null },
      },
      idolB: {
        id: idolB.id,
        name: idolB.name,
        group: idolB.group,
        photo: photoB ? { id: photoB.id, imageUrl: photoB.imageUrl } : { id: null, imageUrl: null },
      },
    };
  }),
});

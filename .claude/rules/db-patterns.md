...
## created_at と updated_at を必ず設定する
...
- 値はデフォルト値を入れるようにする
- insert 時や update 時に手動で設定しない（スキーマの `defaultNow` や `$onUpdate` に任せる
...
Drizzle ORM でレコードを insert する際は、`id` と `created_at` と `upated_at` を**必ず省略**する。

これらはスキーマで自動生成されるため、明示的に渡してはならない。

```ts
// ✅ CORRECT: id と created_at と　updatedAt を省略
